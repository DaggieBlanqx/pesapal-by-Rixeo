...
class PesaPal_Pay {
	...
	*/
	...
	function __construct() {
		...
		add_action("admin_menu", array(&$this, 'admin_menu'));
                ...
                /*Pesa Pay Title filter
                 * Commented off because it displays the code on post, page and comment titles.
                 * 
                 */
		#add_filter('the_title', array(&$this, 'hide_pesapal_transaction_title'));

		add_filter('the_content', array(&$this, 'show_pesapal_transaction_details'));
		add_filter('the_content', array(&$this, 'show_transaction_summary'));
		add_filter('the_content', array(&$this, 'close_transaction_summary_confirmed'));
		...
		if($_REQUEST['action'])
		add_filter('the_content', array($this, 'save_transaction'));

		//Ajax actions to run IPN Return
		...
		if($isLive)
                    $api = "https://www.pesapal.com/api";
		else
                    $api = "https://demo.pesapal.com/api";
		
		$this->post_url = $api. "/PostPesapalDirectOrderV4";
		$this->status_request = $api. "/querypaymentstatus";
			...
			if (is_array($results) && count($results) > 0) {
				...
				foreach ($results as $result) {
					...
					if ($new_post_id) {
						...
						$pesapal_pay_info[0]['payment_method'] = $result->payment_method;
	...
	*/
	function custom_post_type() {
		...
		register_post_type('pesapal_pay', array(
		...
		register_post_status('order_placed', array(
			'label' => _x('Placed Orders', 'post'),
	...
	*/
	function post_status_lables($views) {
		...
		$views['order_placed'] = '<a href="' . $edit_url . '&post_status=order_placed">Placed</a>';
	...
	*/
	function manage_orders_custom_columns($column, $id) {
		...
		//unserialize using maybe_unserialize wordpress function
			...
			switch ($column) {
				...
				case "pp_orders_status":
					...
					if ($post->post_status == 'order_pending')
						...
						$text = __('Trashed');
					else if ($post->post_status == 'order_placed')
						$text = __('Placed');
					?>
				...
				case "pp_orders_payment_method":
					echo $meta["pesapal_pay_info"][0][0]['payment_method'];
					break;
			}
	}
	...
	*/
	function change_order_status() {
		...
		if ($post) {
			...
			}else if ($current_status === "order_pending") {
					$order_status = 'Paid';
	...
	*/
	function save_transaction() {
		...
		$this->save_order('order_placed', $form_invoice, $form_email, $firstname, $lastname, $form_cost, $user_info); //Save Order
		...
		$output = '<div id="pespal-iframe" style="position:relative !important;">';
		
		if( (empty($form_cost) && empty($email)) || (empty($form_cost) && empty($phonenumber)) || (empty($email) && empty($phonenumber)) ){			
			$output .= '<iframe src="' . $iframe_src . '" style="position:relative;overflow:hidden;overflow-x:hidden;overflow-y:hidden;width:100%;top:0px;left:0px;right:0px;bottom:0px;padding:10px;" scrolling="no" frameBorder="0" >';
			$output .= '</iframe>';
			
		}else{
			$output .= '<iframe src="' . $iframe_src . '" style="position:relative;overflow:hidden;overflow-x:hidden;overflow-y:hidden;height:700px;width:100%;top:0px;left:0px;right:0px;bottom:0px;padding:10px" scrolling="no" frameBorder="0"  >';
			$output .= '</iframe>';
		}
	...
	*/
	function save_transaction_with_values($firstname, $form_email, $form_cost) {
		...
		$this->save_order('order_placed', $form_invoice, $form_email, $firstname, $lastname, $form_cost); //Save Order
		...
		if( (empty($form_cost) && empty($email)) || (empty($form_cost) && empty($phonenumber)) || (empty($email) && empty($phonenumber)) ){
			$output = '<iframe src="' . $iframe_src . '" style="position:absolute;overflow:hidden;overflow-x:hidden;overflow-y:hidden;width:100%;top:0px;left:0px;right:0px;bottom:0px;" scrolling="no" frameBorder="0" >';
			$output .= '</iframe>';
		}else{
			$output = '<iframe src="' . $iframe_src . '" style="position:absolute;overflow:hidden;overflow-x:hidden;overflow-y:hidden;height:700px;width:100%;top:0px;left:0px;right:0px;bottom:0px;" scrolling="no" frameBorder="0" >';
			$output .= '</iframe>';
		}
		echo $output;
		exit();
	...
	*/
	function ipn_return() {
		...
		include_once 'pesapal_pay/lib/checkStatus.php';
		...
		if (!empty($transaction_tracking_id) && $payment_notification == "CHANGE") {
			...
			if ($page) {
				...
				$my_old_post_meta = get_post_meta($page->ID,'pesapal_pay_info'); 

				$my_new_post_meta = $my_old_post_meta[0];
				$my_new_post_meta['payment_method'] = $payment_method;

				wp_update_post($my_post);
				update_post_meta($page->ID, 'pesapal_pay_info', $my_new_post_meta);
			}
	...
	*/
	function ipn_page_return() {
		...
		if (!empty($transaction_tracking_id)) {
			...
			//$status = $pesapal->checkStatusByMerchantRef($invoice);
			$paymentDetails = $pesapal->getTransactionDetails($invoice, $transaction_tracking_id);
			$status = $paymentDetails['status'];
			$payment_method = $paymentDetails['payment_method'];
			...
			if ($page) {
				...
				//Update payment_method in the merchants' post_meta for this transaction
				$my_old_post_meta = get_post_meta($page->ID,'pesapal_pay_info');  
				$my_new_post_meta = $my_old_post_meta[0];
				$my_new_post_meta['payment_method'] = $payment_method;
	...
	/**
	*hide the post title that shows invoice number as title.
	function hide_pesapal_transaction_title( $content ) {

		//Appending this function to the content ensures the function is put at the bottom of the page for faster loading.
		$content .= '<script type="text/javascript">';

		$content .= '	jQuery(function($){ ';

		$content .= '		$("header.entry-header").hide(0) ';
	...
	*/
	...
	/**
	*Show other details of a transaction when admin clicks view link on pesal pay section.
	*/
	function show_pesapal_transaction_details( $content ) {

		$transaction_id = $_REQUEST['pesapal_pay'];
 		$options = $this->get_options();

   		$currency = $updated_status = "";
		$currency = $options['currency'];
		
		$transaction = $this->get_transaction($transaction_id);
		$post_id = $transaction->ID;
		$post_status = $transaction->post_status;
	    ...
	    if ( is_single() && 
	    	in_the_loop() &&
	    	(strlen($transaction_id) >= 20) &&
	    	($post_type == 'pesapal_pay') &&
	    	(current_user_can('editor') || current_user_can('administrator')) 
	    ) {
	    	
			/**
			*[0]helps access the array portion that has the data we want.
			*/
			
			$transaction_meta = get_post_meta($post_id,'pesapal_pay_info')[0];
			
			$post_email = $transaction_meta[0]['email'];
			$post_fname = $transaction_meta[0]['fname'];
			$post_lname = $transaction_meta[0]['lname'];
			$post_total = $transaction_meta[0]['total'];   
			$post_payment_method = $transaction_meta['payment_method'];   

			$senders_name = ucwords($post_fname." ".$post_lname);

			//get a better label for payment status
   			switch ($post_status) {
			...
			?>
		   		<fieldset>
	      			...
	      			<table class="widefat">
      					...
      					<tr>
      						<th colspan="2"><center><?php _e('Payment Details')?></center></th>
      					</tr>
      					
      					<tr>
      						<th><?php _e('Reference Number')?></th>
      						<td>
      							<?php _e($transaction_id)?>
      						</td>
      					</tr>
      					...
      					<tr>
      						<th><?php _e('Amount Paid ('.$currency.')' )?></th>
      						<td>
      							<?php _e( number_format($post_total) )?>
      						</td>
      					</tr>
      					...
      					<tr>
      						...
      						<td>
      							<?php _e($updated_status)?>
      						</td>
      					</tr>

      					<?php 

      						foreach ($transaction_meta[0]['user_info'] as $key => $value) {
      							# code...

      							if(is_array($value) && count($value) > 0 && ($value['visible'] == 'checked') ){
      								
      								if($value['value'] == "on" || $value == 1)
      									$value['value'] = "YES";
      								elseif($value['value'] == "off")
      									$value['value'] = "NO";
						?>
									<tr>
			      						<th> <?php _e(ucwords($value['name'])) ?> </th>
			      						<td> <?php _e(ucwords($value['value'])) ?> </td>
			      					</tr>
	...
	*Shows transaction summary on the return page before the content
	*/
	
	function show_transaction_summary( $content ) {
		
   		/**
   		*put the summary only on the return page only if 
   		*1. Server redirected you after making apayment.
   		*2. The Invoice length is above 20 to match md5() equivalence
   		*/
   		$options = $this->get_options();

   		$currency = $updated_status = $transaction_response = "";
		$transaction_response_style = "padding:10px; margin: 20px 0;";

		if( (strlen($_REQUEST['id']) >= 20) ){ 
			...
			*/
			...
			$senders_name = ucwords($post_fname." ".$post_lname);
   			...
   			switch ($post_status) {
				...
				case 'order_pending':
					...
					$transaction_response_style .= "border-left:5px solid grey; background:#eae9e9;";
					
					$transaction_response =  "<h4>Payment Pending</h4>";
					$transaction_response .= "Thank you <b>".$senders_name."</b>, Your payment is being processed.";
                    $transaction_response .= "<br/>";
					$transaction_response .= "Once confirmed, You will receive an Email/SMS notification, and your payment settled instantly. ";
					break;

				case 'order_paid':
					$updated_status = 'COMPLETED';
					
					$transaction_response_style .= "border-left:5px solid #036f03; background:#e6fde6;";
					
					$transaction_response = "<h4>Payment Completed</h4>";
					$transaction_response .= "Thank you <b>".$senders_name."</b>, your payment has been processed successfully.";
   					break;

   				case 'order_cancelled':
   					$updated_status = 'FAILED';
					
					$transaction_response_style .= "border-left:5px solid red; background:#FFADA9;";
					
					$transaction_response = "<h4>Payment Failed</h4>";
					$transaction_response .= "Sorry <b>".$senders_name."</b>, Your payment has failed. This could be because of several reasons:";
					$transaction_response .= "<br/>";

					$transaction_response .= " <ol style='margin:0 0 5px 30px;'> ";
					$transaction_response .= " <li>The card details you entered are incorrect.</li>";
					$transaction_response .= " <li>Your bank may have blocked online payments.</li>";
					$transaction_response .= " <li>You have insufficient funds in the card/mobile money account you are attempting to use.</li>";
					$transaction_response .= " <li>Your bank may have declined this transaction, kindly check with your bank.</li>";
                    $transaction_response .= " </ol>";

                    $transaction_response .= "Kindly try again or contact support at support@pesapal.com for assistance.";
					break;
			}
		    ...
		    <div id="transaction_summary_confirmed">

	      		<div style="<?php _e($transaction_response_style) ?>" >
	      			<?php _e($transaction_response) ?>
	      		</div>		
	      		
	      		<fieldset>

				<!--redirect to front page where the payment form is-->

	      			<form method="POST" action="<?php _e(get_home_url())?>">
	      				<table class="widefat">
	      				...
	      				<p class="submit" align="center">
	      					<input class='button-primary' type='submit' name='transaction_summary_confirmed' value='<?php _e('Make Another Payment'); ?>'/>
	      					<input class='button-primary' type='button' id='close_transaction_summary_confirmed' value='<?php _e('Close'); ?>' />
	...
	}//end of function show_transaction_summary

	
	function close_transaction_summary_confirmed($content){
		...
		$content .= '		$("#close_transaction_summary_confirmed").css({"margin-left":"5em","background":"green","padding":"1em"}); ';		
		$content .= '		$("#close_transaction_summary_confirmed").click(function(){ ';
		$content .= '			$(this).parent().closest("div").fadeOut("fast"); ';
		$content .= '		}); ';
	...
	*/
	function get_transaction($invoice) {
		return get_page_by_title($invoice,'','pesapal_pay');
	}
	...
	*/
	function set_up_meta_box() {
		...
		foreach ($post_types as $post_type) {
			add_meta_box(
				'pesapalpay_sectionid', __('PesaPal Pay Options'), array(&$this, 'post_inner_custom_box'), $post_type, 'side', 'high'
			);
		}
	...
	*/
	function content_filter($content) {
		...
		//put the form only on the front page when there is no transaction summary
		//if( is_front_page() && (strlen($_REQUEST['id']) < 20)){
                //    $content = $content . do_shortcode("[pesapal_pay_payment_form button_name='Buy Using Pesapal' amount='10']");
		//}
...
function general_settings() {
	...
	if (@$_POST['pesapal_settings']) {
		...
		$required_fields['is_live'] = $_POST['is_live'];
	...
	<form method="POST" action="">
		<table class="widefat">
			<tr>
				<td>Use Live Pesapal Account?</td>
				<td>
					<label>
					Live
					<input type="radio" name="is_live" value="1" <?php echo $options['is_live'] ? 'checked':'';?>>
					</label>

					<label>
					Demo
					<input type="radio" name="is_live" value="0" <?php echo $options['is_live'] ? '':'checked';?>>
					</label>
		...
		<div id="dpsc_checkout">
			<div class="dpsc_checkout_fields">
				<form method="POST" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
					...
					<p class="submit">
						<a class="button button-secondary" onclick="add_checkout_element()"><?php _e('Add Field'); ?></a>
						&nbsp;&nbsp;&nbsp;
						<input class='button button-primary' type='submit' name='pesapal_pay_checkout_settings' value='<?php _e('Save Options'); ?>'/><br/>
					</p>
		...
		if (is_array($pesapal_pay_checkout_settings) && count($pesapal_pay_checkout_settings) > 0) {
			...
			foreach ($pesapal_pay_checkout_settings as $checkout_row) {
				...
				if ($checkout_row['visible'] == 'checked') {
					...
					if($checkout_row['name'] === 'Amount')
						$checkout_row['name'] = $checkout_row['name'].$curr_str;

						
					
					if (!empty($default_amount) && $checkout_row['uname'] == 'pesapal_pay_amount') {
				
						$input_elem = '<input type="hidden" class="' . $form_class . ' ' . $checkout_row['uname'] . '_input" name="' . $checkout_row['uname'] . '" ' . $required_attr . ' value="' . $default_amount . '" id="' . $checkout_row['uname'] . '" /> ' . $default_amount;
